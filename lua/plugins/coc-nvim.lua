return {
  ...
  config = function()
    ...
    vim.keymap.set('x', '<leader>lR', '<Plug>(coc-codeaction-refactor-selected)',
      { desc = "Refactor selected", silent = true })
    vim.keymap.set('n', '<leader>lq', '<Plug>(coc-fix-current)', { silent = true, desc = "Quickfix" })
    vim.keymap.set('n', '<leader>lf', '<cmd>call CocAction("format")<CR>', { silent = true, desc = "Format buffer" })
    ...
    vim.g.coc_global_extensions = { 'coc-tsserver', 'coc-json', 'coc-html', 'coc-css', 'coc-emmet', 'coc-prettier',
      'coc-eslint', 'coc-sumneko-lua', 'coc-html-css-support', 'coc-pairs', 'coc-snippets' }
